...
> 🐙 **ALWAYS try to collect `devops_findings` (Defender-for-Cloud DevOps recommendations).** By design the run should **attempt** the `devops_findings` ARG query (option 2 below — narrowest is `RunInTerminal` `az rest --method post` to ResourceGraph) so section 3 populates with the Defender DevOps recommendations (dependency CVEs, CodeQL, IaC, secrets) + repo × severity matrix whenever a connector exists. **When it comes back empty** (no `Microsoft.Security/securityConnectors` DevOps connector in the tenant, or the dataset wasn't collected), section 3 renders a **configure banner** (call-to-action with a Portal · DevOps security link + the GitHub onboarding docs) — it no longer vanishes silently. Check connectors with `GET /subscriptions/{sub}/providers/Microsoft.Security/securityConnectors?api-version=2023-10-01-preview`.
...
>    - `mdc_assessments` ← `GET /subscriptions/{sub}/providers/Microsoft.Security/assessments?api-version=2021-06-01&$expand=metadata` (**`$expand=metadata` is required** — without it `properties.metadata.severity` is absent → every DfC recommendation shows severity **"Unknown"** and the 🛡️ Defender for Cloud **severity pie doesn't render**; it also fills remediation/categories/MITRE)
