...
> ⚠️ **This skill runs in an environment WITHOUT direct Sentinel MCP Server or Microsoft Graph MCP.**
...
- ❌ Using `mcp_microsoft_se2_*` (Sentinel MCP) or `mcp_mtp_mcp_servi_*` (Defender MCP) or `mcp_microsoft_ent_*` (Graph MCP) — these are NOT available in this environment. Note: `monitor-client_monitor_workspace_log_query` (Azure Monitor MCP) IS available and used for KQL in Mode B prefetch
