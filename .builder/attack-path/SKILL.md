...
| `identity_anomaly` | identity-anomaly-score feed · UEBA identities with a **corroborated REAL risk** (IP↔IOC / risky-success sign-in / active alert) | **UEBA → active behaviour**: a corroborated-real identity becomes an entry vector and marks every path that traverses it as **🔴 ativo agora** (theoretical → real), promoting it to the "o que importa" tier with a `🔴 UEBA:` evidence chip. `rr_klass: real` = active; `suspect` is tagged only. |
...
- **Hero KPIs (importance-first):** `🔥 o que importa · 🔴 ativos agora · 🧬 cross-domain · 👁️ sem detecção · 📋 só recomendação` + verdict (CRÍTICA / ELEVADA / MODERADA / CONTIDA).
- **🔥 Signal vs noise (the headline):** the report is split by **importance**, not raw risk. A path is **importante** when it is a toxic combination **no product correlates** (cross-domain), it leads to **takeover**, or it has **active behaviour** (hunt hit). The top section **"🔥 O que realmente importa"** shows only those. A single-hop misconfig that Defender/Advisor already lists (e.g. a lone public-access or open-ports recommendation) is **"📋 só recomendação"** and is demoted to a **collapsed "Recomendações conhecidas" section** as context — not noise at the top. Each window carries a tier badge (🔥 crítico · ⚡ relevante · 📋 recomendação). `classify_importance()` sets the tier; chokepoints + "Por onde começar" are computed over the **important** subset only.
- **🎯 "Por onde começar" callout:** one plain line at the top — the single fix that closes the **most** *important* attack paths at once (with a deep link to open the object). The blast-radius prioritisation lives inline and in context (no abstract table).
- **🪟 Expandable path windows (one self-contained story each):** every attack path is a native `<details>` card — collapsed by default for a clean, intuitive page. Expanding shows: its **own layered mini-map** (`🌐 Acesso externo → 🚪 Entrada → ↪ Pivô → 🎯 Alvo — até onde chega`; boxes coloured by role, edges by risk, dashed yellow = blind spot, ATT&CK technique per edge); the **🔧 correção with a deep link to open the object** (Entra app/user · Defender device · Azure resource), the **📋 recomendação** real do Defender for Cloud (when the entry is an exposure), and a **💥 "fecha N caminhos"** badge; the **🔬 directed hunting** for that path; and (when active) the **🚨 Sentinel incident proposal**.
- **🔬 Directed hunting (Module 5):** for every edge relation, the skill emits the *appropriate* live hunt on the path's real entities (SP sign-ins / credential-added · Graph activity · risky sign-ins / inbox rules · role changes · device alerts / admin logons). If the bundle carries `hunt_results` (or you pass `--hunt` to run them live), confirmed behaviour lights up **🔴 ativo agora** and the path is flagged active. The **`identity_anomaly` feed** does the same by behaviour: any path through a UEBA identity whose risk is **corroborated-real** is flagged active (no hunt needed) and shows a `🔴 UEBA:` chip with the corroborators. Hunts are `sentinel` (KQL) or `xdr` (Advanced Hunting via Graph runHuntingQuery).
