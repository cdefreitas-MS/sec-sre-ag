...
try:  # FIXED: python-dotenv is optional — don't crash if missing
...
def enrich_ips(ip_list: list[str], max_workers: int = 3) -> list[dict]:
    ...
    # FIXED: show which APIs are configured (transparency for the user)
    configured = []
    if config.get('ipinfo_token'):
        configured.append('ipinfo.io')
    if config.get('abuseipdb_token'):
        configured.append('AbuseIPDB')
    if config.get('vpnapi_token'):
        configured.append('vpnapi.io')
    if config.get('shodan_token'):
        configured.append('Shodan (full)')
    else:
        configured.append('Shodan InternetDB (free)')
    
    print(f"Enriching {len(ip_list)} IPs...")
    print(f"  APIs configured: {', '.join(configured) if configured else 'None (InternetDB only)'}\n")
