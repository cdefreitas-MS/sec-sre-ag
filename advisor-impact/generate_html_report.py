...
def analyze_mdc(data, rmap, q, rg, include_healthy, sub_names=None):
    ...
    for a in as_list(data):
        ...
        item = {
            ...
            "user_impact": meta.get("userImpact", "") or "",
            "impl_effort": meta.get("implementationEffort", "") or "",
        }
        out.append(_enrich(item, rid, risk, rmap, q, sub_names=sub_names))
    return out
...
def _render_mdc_section(ctx):
    ...
    return (
        ...
        "<h3 style='margin-top:16px'>🔧 Recomendações <span class='meta'>· ordenadas por potencial de elevação do Secure Score (maior → menor) · com impacto no workload (userImpact) e custo de aplicar</span></h3>"
        "<div id='mdctable'></div>"
        "</div></div>")
...
const MDC_IMPACT_COLOR={"High":"#ff6b6b","Moderate":"#ffd96b","Low":"#7ee2a8"};
        ...
        mdc_js = {
            ...
            "recs": [
                {"title": it.get("title") or "—",
                 ...
                 "resource_type": it.get("resource_type") or "",
                 "link": it.get("portal_link") or it.get("rec_link") or "",
                 "sub": it.get("subscription_id") or "",
                 "rg": it.get("resource_group") or "—",
                 "cost": it.get("cost_increase") or "",
                 "user_impact": it.get("user_impact") or "",
                 "impl_effort": it.get("impl_effort") or "",
